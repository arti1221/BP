...
def generate_random_code():
    while (Room.objects.filter(code=code).count != 0):
        code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=Room.code_length))
    return code
...
class Room(models.Model):
    code = models.CharField(max_length=8, default="", unique=True) # Room Code to join to
